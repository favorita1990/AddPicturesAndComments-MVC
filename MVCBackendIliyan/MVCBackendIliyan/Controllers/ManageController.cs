...
namespace MVCBackendIliyan.Controllers
{
    ...
    public class ManageController : Controller
    {
        ...
        public async Task<ActionResult> _PartialIndex(ManageMessageId? message)
        {
            ViewBag.StatusMessage =
                message == ManageMessageId.ChangePasswordSuccess ? "Your password has been changed."
                : message == ManageMessageId.SetPasswordSuccess ? "Your password has been set."
        ...
        [HttpGet, ActionName("PartialDeleteAccount")]
        public ActionResult _PartialDeleteAccount()
        {
            MVCBackendDb db = new MVCBackendDb();
            var user = db.Users.Find(User.Identity.GetUserId<int>());
            return PartialView("_PartialDeleteAccount", user);
        }
        ...
        {
            MVCBackendDb db = new MVCBackendDb();
            var userModel = db.Users.Find(User.Identity.GetUserId<int>());
            var forumOfUser = db.Forums.Where(x => x.UserId == userModel.Id);
            ...
            foreach (var item2 in galleryOfUser)
            {
                var fullPath = Server.MapPath("~/Images/" + item2.ImageUrl);
                if (System.IO.File.Exists(fullPath))
                {

                    System.IO.File.Delete(fullPath);
                }
                db.Galleries.Remove(item2);
            }
            foreach (var item in forumOfUser)
            {
            ...
            return RedirectToAction("Index", "Home", new { area = "", message = "User was removed." });
        }
        ...
        [HttpGet, ActionName("PartialChangePassword")]
        public ActionResult _PartialChangePassword()
        {
            return PartialView("_PartialDeleteAccount2");
        }
        ...
        public async Task<ActionResult> ChangePassword(ChangePasswordViewModel model)
        {
            ...
            if (result.Succeeded)
            {
                ...
                return RedirectToAction("PartialIndex", new { Message = ManageMessageId.ChangePasswordSuccess });
            }
            AddErrors(result);
            return PartialView("_PartialChangePassword", model);
        }
